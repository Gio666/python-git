...
def obtenerContenidoDeArchivo():
    # nombre del archivo
    nombreDeArchivo = sys.argv[1]
    # abrimos el archivo
    archivo = open(nombreDeArchivo)
    # leemos el contenido del archivo
    contenidoDeArchivo = archivo.read()
    # lo separamos por lineas en una lista
    listaDeRenglones = contenidoDeArchivo.splitlines(True)
    
    # todo junto
    print (open(sys.argv[1]).read().splitlines(True))
    print (listaDeRenglones[0].split())
    
    tokens = []

def agregarToken(token):
    tokens.append(token)

def getListaDeTokens():
